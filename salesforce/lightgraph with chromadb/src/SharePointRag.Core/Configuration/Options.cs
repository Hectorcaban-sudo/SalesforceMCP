...
/// <summary>
/// Controls batching behaviour during ingestion.
///
/// Two independent batch boundaries work in tandem:
///
///   EmbeddingBatchSize
///     How many chunk texts are accumulated across multiple source records
///     before a single Azure OpenAI embeddings call is made.
///     Larger values reduce API round-trips and improve throughput.
///     Azure OpenAI limit: 2048 inputs per request.
///     Default: 256 (safe for most text-embedding-3-large workloads).
///
///   UpsertBatchSize
///     How many embedded chunks are accumulated before they are written
///     to LiteGraph in a single pass. Each pass is one logical write
///     unit — LiteGraph writes nodes sequentially within the call but
///     the buffer is released immediately after, keeping peak memory low.
///     Default: 100.
///
/// Memory model:
///   Peak memory ≈ EmbeddingBatchSize × MaxTokensPerChunk × 4 bytes (floats)
///              + UpsertBatchSize × avg chunk text length
///   With defaults (256 × 512 tokens × ~4 bytes) ≈ ~500 KB of embeddings
///   in flight at any moment — regardless of corpus size.
/// </summary>
public class BatchingOptions
{
    ...
    /// <summary>
    /// Number of chunk texts buffered across source records before a single
    /// Azure OpenAI embeddings API call is made.
    /// Range: 1–2048. Default: 256.
    /// Set lower if you hit token-count limits on large chunks.
    /// </summary>
    public int EmbeddingBatchSize { get; set; } = 256;

    /// <summary>
    /// Number of fully-embedded chunks accumulated before they are
    /// written to LiteGraph (and released from memory).
    /// Range: 1–10000. Default: 100.
    /// Larger values reduce LiteGraph write overhead; smaller values
    /// reduce peak memory and improve progress visibility in logs.
    /// </summary>
    public int UpsertBatchSize { get; set; } = 100;

    /// <summary>
    /// How long (ms) to wait for the embedding buffer to fill before
    /// flushing whatever is accumulated. Prevents stalling at end-of-source
    /// when fewer than EmbeddingBatchSize chunks remain.
    /// Default: 2000 ms.
    /// </summary>
    public int FlushTimeoutMs { get; set; } = 2000;
}
