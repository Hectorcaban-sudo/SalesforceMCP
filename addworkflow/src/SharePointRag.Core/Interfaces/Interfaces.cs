...
//  Abstracts which backend (LiteGraph / Chroma) creates each IVectorStore.
//  The LibraryRegistry calls this instead of newing up a concrete type.
// ═══════════════════════════════════════════════════════════════════════════════

/// <summary>
/// Creates an IVectorStore for a specific RAG system.
/// Concrete implementations: LiteGraphVectorStoreFactory, ChromaVectorStoreFactory.
/// The active factory is selected by VectorStoreOptions.Provider in configuration.
/// </summary>
/// <summary>
/// Creates an IVectorStore scoped to a single (system, data source) pair.
/// Each data source within a system gets its own isolated store so that
/// per-source metadata schemas stay clean and queries can target a single source.
/// </summary>
public interface IVectorStoreFactory
{
    /// <summary>
    /// Create a vector store scoped to one specific data source within a RAG system.
    /// Naming convention: "{systemName}__{dataSourceName}" ensures global uniqueness.
    /// </summary>
    IVectorStore Create(RagSystemDefinition system, DataSourceDefinition dataSource);
}
...
{
    /// <summary>The RAG system this store belongs to.</summary>
    string SystemName { get; }

    /// <summary>
    /// The specific data source this store is scoped to.
    /// Empty string for composite stores that span multiple sources.
    /// </summary>
    string DataSourceName { get; }
...
public interface ILibraryRegistry
{
    ...
    /// <summary>
    /// Returns a composite IVectorStore that fans search across all per-source stores
    /// for the given system. Upsert/Delete are routed by chunk.DataSourceName.
    /// </summary>
    IVectorStore GetVectorStore(string systemName);

    /// <summary>
    /// Returns the isolated IVectorStore for one specific data source within a system.
    /// Use this when you need to query or inspect a single source's store directly.
    /// </summary>
    IVectorStore GetVectorStore(string systemName, string dataSourceName);
