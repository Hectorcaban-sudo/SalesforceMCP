...
/// <summary>
/// ChromaDB-backed vector store scoped to a single (system, data source) pair.
///
/// Storage model (one Chroma server, one collection per system × data source):
///   Collection  "{CollectionPrefix}{systemName}__{dataSourceName}"
///               e.g. "rag_General__GeneralDocs",  "rag_Contracts__DeltekVantagepoint"
///     Documents  (one per DocumentChunk)
///       id:        chunk.Id
///       embedding: float[]
///       document:  chunk.Content
///       metadata:  { sourceId, dataSourceName, connectorType, title, url, author,
///                    lastModifiedUnix, chunkIndex, totalChunks,
///                    + all source-specific Metadata entries prefixed "src_" }
///
/// Why one collection per (system, data source):
///   - SharePoint docs carry DriveItemId, LibraryPath metadata; SQL records carry
///     arbitrary column values; Deltek records carry EntityType, ProjectNumber etc.
///     Separate collections mean these schemas never collide.
///   - A CompositeVectorStore fans search across per-source collections and merges results.
///   - Delta deletes are simpler: filter by sourceId within the source's own collection.
/// </summary>
public sealed class ChromaVectorStore : IVectorStore
{
    ...
    // Collection name: "{prefix}{systemName}__{dataSourceName}"
    private string CollectionName =>
        $"{_opts.CollectionPrefix}{_system.Name}__{_dataSourceName}";
