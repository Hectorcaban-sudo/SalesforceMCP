...
///   LiteGraphVectorStore  per system  (isolated LiteGraph Graph in shared SQLite)
///   IIndexStateStore      per system
///   IIndexingPipeline     per system  (fans over all assigned connectors)
///
/// New connector types need zero changes here — ConnectorRegistry dispatches automatically.
/// </summary>
public sealed class LibraryRegistry : ILibraryRegistry
{
    ...
    public LibraryRegistry(
        ...
        ILogger<LiteGraphVectorStore>         storeLogger,
        ...
        ILogger<IndexingPipeline>             pipelineLogger)
    {
        ...
        {
            // Validate referenced sources exist
            ...
            // LiteGraph: one graph per system, all in the same SQLite file
            var store = new LiteGraphVectorStore(
                sys, liteGraphOpts.Value, aoaiOpts.Value, storeLogger);

            // JSON state store: tracks indexing progress per source
            var state = new JsonFileIndexStateStore(
                sys.Name,
                Path.GetDirectoryName(liteGraphOpts.Value.DatabasePath)
    ...
    public async Task<List<RagSystemStatus>> GetAllStatusAsync(CancellationToken ct = default)
    {
        ...
        foreach (var sys in _config.Systems)
        {
            ...
            var store  = _stores[sys.Name] as LiteGraphVectorStore;
            ...
            result.Add(new RagSystemStatus(
                sys.Name, sys.Description, healthy, dsStatuses));
        }
