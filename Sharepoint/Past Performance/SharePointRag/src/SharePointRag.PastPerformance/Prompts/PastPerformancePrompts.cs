...
/// <summary>
/// All LLM prompt templates for the Past Performance Agent.
/// Separated from service logic so prompts can be versioned, A/B tested,
/// or loaded from configuration without recompiling.
/// </summary>
public static class PastPerformancePrompts
{
    ...
    public const string QueryParserSystem =
        ...
        Your job is to parse a user's question about past performance into a structured JSON object.

        Output ONLY valid JSON that matches this schema (no markdown fences, no commentary):
        {
          "SemanticQuery":       "<concise keyword-rich phrase for vector similarity search>",
          "Intent":              "<one of: FindSimilarContracts | GenerateVolumeSection | FindReferences | SummarisePortfolio | IdentifyGaps | ExtractCPARSRatings | FindKeyPersonnel | General>",
    ...
    public const string ContractExtractionSystem =
        ...
        - Use null for any field not mentioned in the source text — never hallucinate values.
        - Dollar values must be numbers (no $ signs or commas).
        - Dates must be ISO 8601 (YYYY-MM-DD).
        - If CPARS ratings are mentioned with adjectives like "outstanding" normalise to "Exceptional".
        - KeyAccomplishments should be specific and measurable (e.g. "Delivered 99.97% system uptime").
        """;
    ...
    public const string NarrativeDraftSystem =
        ...
        10. Flag with [VERIFY] any field that was unclear in source documents.
    ...
    // ── Executive summary (volume cover) ─────────────────────────────────────
    ...
    public const string ExecutiveSummarySystem =
        ...
        Highlight total relevant experience, strongest CPARS ratings, and why the
        team is uniquely qualified for this opportunity.
    ...
    public const string ExecutiveSummaryUserTemplate =
        ...
        The following contracts are included in the volume:
        {contractSummaries}
        """;
}
