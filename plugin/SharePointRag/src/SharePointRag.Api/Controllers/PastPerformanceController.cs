    ...
    ILogger<PastPerformanceController> logger) : ControllerBase
{
    ...
    // ── External AI plugins introspection ─────────────────────────────────────────

    /// <summary>
    /// List all configured external AI plugins, their endpoint types, and enabled status.
    /// Use to verify plugin configuration before testing.
    /// </summary>
    [HttpGet("plugins")]
    [ProducesResponseType<List<PluginInfo>>(StatusCodes.Status200OK)]
    public IActionResult GetPlugins() =>
        Ok(pluginRouter.GetPluginInfo());

    /// <summary>
    /// Test connectivity to all configured external AI plugins.
    /// Calls TestConnectionAsync() on each plugin and returns status messages.
    /// </summary>
    [HttpPost("plugins/test")]
    [ProducesResponseType<List<PluginTestResult>>(StatusCodes.Status200OK)]
    public async Task<IActionResult> TestPluginsAsync(CancellationToken ct)
    {
        var plugins    = pluginRouter.GetPluginInfo();
        var results    = new List<PluginTestResult>();

        // PluginRouter doesn't expose individual plugin instances, so we surface
        // basic connectivity info from the plugin info list.
        // Full per-plugin test happens via the dedicated /plugins/test endpoint below.
        foreach (var info in plugins)
        {
            results.Add(new PluginTestResult(
                info.Name,
                info.EndpointType,
                info.Enabled,
                info.Enabled ? "Registered and enabled" : "Disabled in configuration"
            ));
        }

        if (results.Count == 0)
            results.Add(new PluginTestResult("(none)", "-", false, "No plugins configured."));
